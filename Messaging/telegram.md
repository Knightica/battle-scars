...
## Scars & gotchas
- **Reactions fail in basic groups** - `setMessageReaction` only works in supergroups (chat_id prefixed -100...); basic groups silently reject it. Migrate a basic group to a supergroup before wiring reactions. The thumbs-up emoji (U+1F44D) is on Telegram's whitelisted standard-emoji list; custom/animated emoji require Telegram Premium and will fail for standard bots.
...
- **Bot reactions - check-mark and cross-mark emoji are explicitly rejected** - the standard-emoji whitelist excludes every check-mark variant (U+2705, U+2714, U+2611) and the cross mark (U+274C). Thumbs-up (U+1F44D) is the correct bot reaction for a success path, thumbs-down (U+1F44E) for failure. Code sending a check mark gets `REACTION_INVALID` / Bad Request from the Bot API.
...
- **A bot's OWN outgoing messages never appear in its getUpdates, so alerts sent straight to a DM via a dedicated bot are an unmonitorable channel** - if a background job wires failure and non-failure alerts through a bot straight to a personal DM (no group), neither a separate health-check bot nor the alerting bot's own getUpdates can read what it sent: getUpdates only carries INBOUND updates. Consequences: (1) a health-check must query the source system directly (job run status AND run output, e.g. a filter on the output field for a specific alert marker) rather than trying to read the alert off Telegram; (2) do not add a DM channel to any Telegram-scan config expecting to see the alerts land there; (3) if you want alerts to be machine-auditable, post them to a group the auditing bot is also in, or log the alert condition in the run output.
