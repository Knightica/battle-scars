...
- **`optimization_goal` is IMMUTABLE once an ad set is published - get it right at creation or rebuild** - `POST /<adset_id>` with a changed `optimization_goal` fails with `error_subcode 3260011`, "You can't edit your pixel, conversion event, custom conversion or optimization for an ad set after the ad set is published. To run an ad set with your desired changes, create a new ad set." The same lock covers `promoted_object` (pixel id, custom_event_type, custom conversion). Consequence: a mis-set optimization goal cannot be corrected in place, only by building a NEW ad set, which starts learning from zero. Plan the goal against realistic event volume BEFORE publishing: VALUE is the most data-hungry goal Meta offers and needs many multiples of the 50-conversions-per-week learning floor, so on a small account it locks the ad set into permanent LEARNING with no in-place escape.

- **Campaign-level `stop_time` returns `{"success":true}` and silently does NOT persist on a daily-budget campaign** - `POST /<campaign_id>` with `stop_time` is accepted, `updated_time` on the campaign even advances, but re-reading the campaign shows no `stop_time` field at all. The schedule for a daily-budget (non-lifetime) campaign lives on the AD SET as `end_time`, not on the campaign. Always re-read the object after any scheduling write instead of trusting the success envelope: Meta's `{"success":true}` means "request accepted", not "field stored".

- **Write `end_time` as a Unix epoch, never ISO-8601 with an offset - the rejection message lies** - `POST /<adset_id>` with an ISO-8601 `end_time` carrying a UTC offset (e.g. `2027-01-15T23:59:00+0200`) fails with `error_subcode 1487033`, `blame_field: time_stop`, and the user-facing message "End Date Is In the Past. Your campaign's end date must be in the future" - even when the date is weeks in the FUTURE. Meta misparses the offset form and then reports the result as a past-date error, which sends you looking at your dates instead of your format. The identical request with a Unix timestamp (`end_time=1789948740`) returns success and persists correctly. Compute the epoch in the AD ACCOUNT's timezone, readable from `GET /<ad_account_id>?fields=timezone_name`.

- **Account `spend_cap` is WRITTEN in whole currency units but READ back in minor units - and an active cap silently switches off every campaign when hit** - `POST /act_<id>` with `spend_cap=600000` (intending the number `GET` returns, which is actually minor units) sets the limit 100x too high; the correct write is the major-unit figure (e.g. `spend_cap=6000` for 6,000 in the account's currency), after which `GET /act_<id>?fields=spend_cap` returns `600000`. Same field, different units per direction, and the success envelope gives no hint - always re-read after writing. Second half of the scar: an account spending limit is not a pacing tool. One account hit its spend cap and every campaign went dark for a week with no alert, no failed run, nothing red in Ads Manager; the only trace is `ad_account_billing_charge` / `ad_account_update_spend_limit` / `ad_account_reset_spend_limit` events in `GET /act_<id>/activities` and zero daily spend in insights. When auditing a period with unexpectedly low spend, pull daily account insights and the activities log before blaming delivery.
