...
- **Restoring a deleted file re-uploads its entire blob on the next push, even though the remote already stores that exact blob.** Delete a large file in one commit, push, then bring it back in a later commit (`git checkout <old-sha> -- <path>`, a revert, or a restore from a tag): the next push sends the full content again. Verified with a control: a 3MB blob already present in the bare remote and reachable from an earlier commit, restored in a new commit, produced `Writing objects: 100% (3/3), 2.86 MiB` on push. Git's pack computation only marks objects uninteresting via the trees it can prune, and a restored file always sits in a freshly-built tree, so the blob gets enumerated and re-sent. Consequence: a "revert the deletion" commit costs the same uplink as the original import. On a slow uplink, restoring a folder of many files (tens of MB) is not a cheap operation, it is a full re-upload. Build the commit server-side instead (next scar).

- **The GitHub Git Data API builds a commit entirely server-side, so a restore costs zero upload.** When the content already exists in the repo (an older commit, a tag), never push it back from the laptop. Reference the existing SHAs and let GitHub assemble the objects:
  1. `GET /repos/{owner}/{repo}/git/trees/{tag-or-sha}?recursive=1` to read the tree you want back.
  2. `POST /repos/{owner}/{repo}/git/trees` with entries whose `sha` points at the existing blobs (pass `sha`, never `content`, or you are uploading again). Use `base_tree` to layer the restore over the current tree.
  3. `POST /repos/{owner}/{repo}/git/commits` with that `tree` and the current head as `parents`.
  4. `PATCH /repos/{owner}/{repo}/git/refs/heads/{branch}` with the new commit `sha`.
  The request bodies are a few KB regardless of how many megabytes the tree holds. Afterwards `git fetch` locally, because your clone does not know about the new commit. Caveat: this bypasses local hooks and lands directly on the branch, so it respects branch protection (a protected branch still requires the PR flow) but skips any pre-commit checks.
