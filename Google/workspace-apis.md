...
- **Changing a user's primary address breaks their Gmail sending until the sendAs default is re-picked** - after an admin renames a user's primary email (e.g. yuval@pillar-company.com -> yuval@pillar-company.co.il), Gmail compose keeps the OLD address as the default "Send mail as" identity, and every send fails with "The 'From' address is invalid. Please choose a valid one." Receiving is unaffected (the old primary auto-becomes an alias). Fix, in the USER's Gmail: Settings -> Accounts -> "Send mail as" -> set the new address as default (delete the stale entry if it errors), and in any already-open compose click the From line and re-pick - open drafts keep the stale identity. Via API this needs `gmail.settings.basic` (sendAs update), which a minimal gmail.modify DWD grant does not cover. Second variant, same error, sendAs list already CORRECT: the Accounts setting "Reply from the same address the message was sent to" makes Gmail set From = the old primary on replies to pre-rename threads, and the old primary is a receive-only alias, not a send identity. Fix: switch to "Always reply from default address", or add the old address via "Add another email address" as an alias to restore reply-from-same. Already-open drafts keep the dead From either way - re-pick it on the From line. Hit twice: Maru on knightica.com (fix never written down), Yuval on pillar-company (2026-09-02, the reply-from-same variant) - the second occurrence cost a full re-diagnosis because the first was never recorded.
